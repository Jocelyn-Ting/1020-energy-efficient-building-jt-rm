function building = addLayout(layoutNum)
    ...
    %layout 4 is layout 1 but with even smaller insulation to help release heat in the
    %summer
    if layoutNum == 4
        % Building Materials
        RSI_interior_wall = .1*RSI_concrete+.05*RSI_fiberglass;
        RSI_exterior_wall = .1*RSI_concrete+.05*RSI_fiberglass+.1*RSI_concrete;
        RSI_roofing = 0.1*RSI_concrete+0.1*RSI_fiberglass+RSI_asphalt_roof;
